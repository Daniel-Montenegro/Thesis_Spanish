...
\figcaption{\emph{Esquema del sistema donde se ha dado una región warp. Se observan los diferentes espines de cada objeto, el radio warp, radio de la LSO. [SEBAS]}}\label{fig: sistema con zona warp}
\end{center}
...
\figcaption{\emph{Comparación entre un sistema que co-rotante (b) y contra-rota (a). Se observa que cuando es contra-rotante ($a\to -1$)} el radio $r_{lso}\to 9$, y de forma contraria si es co-rotante ($a\to 1$) el radio $r_{lso}\to 1$.}\label{fig: contra-rotacion y co-rotacion}
\end{center}
\begin{center}
\includegraphics[scale=.5]{./figures/4_Modelo_Spin/eficiencia_radiativa.pdf}
\figcaption{\emph{Dependencia entre el radio de ultima orbita estable $r_{lso}$ con la eficiencia radiativa $\epsilon_{lso}$, se observa que cuando $r_{lso}$ es mínimo la eficiencia es máxima, que corresponde a la máxima co-rotación $a=1$, indicando que gran parte del material se pierde y no contribuye al espín del sistema. Al observa el otro caso, pasa todo lo contario, siendo el sistema contra-rotante $a=-1$ el que mejor contribuye a la evolución del espín.}}\label{fig: eficiencia_radiativa }
\end{center}
%%************************
...
\figcaption{\emph{ Proceso evolutivo de los espines del sistema. En la figura superior, se tiene el caso inicial donde se formó la región warp, la cual presenta precesión. La figura del medio presenta el momento en que la región warp se estabiliza, alineando su espín con el espín del BH. La figura inferior muestra el momento final, donde todo los espines se alinean [SEBAS].}}\label{fig: evolucion espin}
\end{center}
...
\figcaption{\emph{Modelo auto-gravitante, donde en la parte exterior a la región warp se forma un disco más grueso de con una dinámica auto-gravitante[SEBAS]}}\label{fig: Sistema auto-gravitante}
\end{center}
%
...
\figcaption{\emph{Sistema binario de BHs, donde cada uno tiene un espín característico y la contribución de cada uno conlleva a un momentum angular del sismtema $\vec{\ell}$ [SEBAS]}.}\label{fig: fusion_Bhs}
\end{center}
